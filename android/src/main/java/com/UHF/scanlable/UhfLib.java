...
public class UhfLib {
	...
	//閻犲洩顕ч崯鎾诲闯閵娧勭暠婵炲鍨绘竟鎺楁偝閸ャ儻鎷烽悹鍥嚙閸熸捇宕抽妸褎鐣遍柛锔芥緲濞硷拷
...
//闁瑰灚鎸哥槐鎴犳嫚鐠囨彃鏅搁柛锝冨妽鑶╅柛褎顨愮槐婵嬬嵁閹澘绠柛娆愮墳椤曚即宕樺▎蹇旂彜闁汇劌瀚穱濠囧箒閿燂拷
...
public int ReGetInfo()
{
  ...
  //閻犱礁澧介悿鍡欐嫚鐠囨彃鏅搁柛锝冨妿濞堟垹鎷犵拠鎻掓櫢闁汇垽娼ч悺娆撳冀閸モ晩鍔柣銊ュ濡炲倿姊绘潏鍓х0x03~0xff
  ...
  //閻犱礁澧介悿鍡欐嫚鐠囨彃鏅搁柛锝冨妿power 0x00闁挎繐鎷穢1e濞堟垿宕濋悢鍝勮姵
  ...
  //闊浂鍋婇敓浠嬪蓟閵夘煈鍤勯梻鍕缁诲酣鎯冮崙鐑禖C1G2
/*  public int EPCC1G2_QuicScanEPC(
			byte QValue,
			byte Session,
			byte Target,
			byte Ant,
			byte ScanTime)
  {
	  if(EPCC1G2_QuickInventoryEPC(uhf_addr,
				QValue,
				Session,
				Target,
				Ant,
  ...
  public byte EPCC1G2_QuicScanEPC_State()
  {
		  return state[0];

  }
  public int EPCC1G2_QuickInventoryEPC_TagNum()
{

		  return pOUcharTagNum[0];
}

  public byte[] EPCC1G2_QuickInventoryEPC_UIDList()
{
		  return pOUcharUIDList;
}
*/

//闁哄被鍎撮妤呮⒔閸曨喚绠柣銊ュ殣PCC1G2
...
//EPC濞戞挶鍊撻柌婊咃拷濡ゅ嫬螡 婵炲鍔嶉崜浼村礆閸℃纾婚柛鎰舵嫹濡ゅ倹锚閻⊙囨嚍閸屾碍韬柛鎿勬嫹
